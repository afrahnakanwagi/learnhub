...
// ✅ All icons properly imported (including Search)
import { 
  Users, BookOpen, Search, Star, DollarSign, Plus, Edit, 
  Trash2, Eye, TrendingUp, Award 
} from "lucide-react";
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={item.label} className="bg-white rounded-2xl border border-gray-100 shadow-card p-5">
              ...
              <p className="text-xs text-green-600 font-semibold mt-1">{item.change}</p>
            </div>
          ))}
        </div>
              ...
              {["earnings", "students"].map(tab => (
                <button 
                  ...
                  className={`text-xs font-semibold px-3 py-1.5 rounded-lg capitalize transition-all 
                    ${activeChart === tab ? "bg-brand-500 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
                >
                  {tab}
                </button>
              ))}
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={activeChart === "earnings" ? earningsData : enrollmentData}>
              ...
              <Bar 
                ...
                fill={activeChart === "earnings" ? "#22c55e" : "#f97316"} 
                radius={[8,8,0,0]} 
              />
            </BarChart>
          </ResponsiveContainer>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-gray-500 flex items-center gap-1">
                      <Users size={11} />{course.students.toLocaleString()} students
                    </span>
                    <span className="text-xs text-gray-500 flex items-center gap-1">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex gap-4 p-4 bg-gray-50 rounded-2xl">
                <div className="w-10 h-10 bg-orange-500 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">
                  {review.avatar}
                </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Are you sure you want to delete <span className="font-bold text-gray-900">"{showDeleteModal?.title}"</span>? 
